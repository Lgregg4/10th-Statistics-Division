...
```
The above plot shows we have alot of data between year 504 and 1996. Unfortunately, in order to consider this data we must drop the information from the VOI rainfall data set.

However, we are able to grab a variable from each data set to plot against the year the data was recorded.

```{r}
match_dataC <- dataC %>% filter(`Age AD` >= 504 & `Age AD` <=1996)
match_dataK <- dataK %>% filter(Year >= 504 & Year<= 1996)
match_dataR <- dataR

ggplot() +
  geom_point(data=match_dataC, aes(x=`Age AD`, y=`varve thickness`, color="varve thickness")) +
  geom_point(data=match_dataK, aes(x=Year, y=`BSi MAR (mgSiO2/cm2y)`, color="BSi MAR (mgSiO2/cm2y)")) +
  geom_point(data=match_dataR, aes(x=year, y=BSi, color="BSi")) +
  labs(x="Year", y="Values", color="Values") 
```

The above plot considers BSi, BSi MAR, and varve thickness from year 505 to 1996.
